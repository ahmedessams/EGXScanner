...
-- ---------------------------------------------------------------------
INSERT INTO markets (code, name, currency, timezone, eodhd_exchange_code, index_code, min_avg_traded_value, min_avg_volume, min_active_days_20, active, min_target_gain_pct, atr_stop_mult, round_trip_cost_pct, momentum_overext_penalty) VALUES
    ('EGX', 'Egyptian Exchange',  'EGP', 'Africa/Cairo',    'EGX', 'EGX30', 500000,   50000,  15, TRUE, 2.00, 2.50, 0.400, FALSE),
    ('US',  'US Market (Nasdaq/NYSE)', 'USD', 'America/New_York', 'US',  'GSPC',  1000000, 300000, 15, TRUE, 1.50, 1.50, 0.050, TRUE)
ON CONFLICT (code) DO UPDATE
    SET name = EXCLUDED.name, currency = EXCLUDED.currency, timezone = EXCLUDED.timezone,
        ...
        momentum_overext_penalty = EXCLUDED.momentum_overext_penalty;
