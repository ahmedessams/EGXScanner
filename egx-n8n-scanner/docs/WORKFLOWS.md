...
| `GET /webhook/egx/top-picks` | Curated Top 3 trade ideas: eligible Top 10 picks whose Target 1 is structurally within reach — setup ACCUMULATION or MOMENTUM, `target1_estimated_days` ≤ 5 and `risk_reward_t1` < 1.5 — first 3 by rank. Never lower than rank 10; fewer (down to zero) on a day nothing clears the bar. Each row still adds `similar_n`, `similar_target1_hit_pct`, `similar_stop_hit_pct`, `similar_expected_value_pct` (context, not a filter). Full-history lab 2026-09-04, Top-3 gated vs ungated: EGX BT 44.0% vs 41.2% hit / +1.01% vs +0.96%; EGX LIVE 51.5% vs 50.0% / +2.33% vs +2.13%; US BT 31.7% vs 30.3% / +0.27% vs +0.24%; US LIVE 46.9% vs 34.4% / +1.65% vs +0.20%. The ≥5% gain floor, the similar-cohort ≥50% gate and the AI ≥40% gate were removed the same day — see SCORING.md "Top 3 Trade Ideas". `?date=YYYY-MM-DD`, `?market=` |
